...
export function ProfileHeader({
  ...
  return (
    <div className="">
      <div className="w-full h-32 md:h-48 bg-gray-800 rounded-lg overflow-hidden">
        <img src={bannerUrl} alt="Banner" className="w-full h-full object-cover" />
      </div>

      <div className="max-w-6xl mx-auto px-4 py-6 flex flex-col md:flex-row gap-4">
        <img src={avatarUrl} alt={username}
          className="w-24 h-24 md:w-32 md:h-32 rounded-full border-4 border-green-600"/>
        <div>
          ...
          <div className="mt-4 flex gap-2">
            ...
            <>
              <button
                ...
                <SettingsIcon sx={{ fontSize: "18px", color: "black", ml: "5px" }} />
                Editar perfil
              </button>
              <button
                onClick={onDeleteProfile}
                className="bg-red-500 text-white border px-4 py-1 rounded flex items-center gap-2">
                <RemoveCircleIcon sx={{ fontSize: "18px", color: "black", ml: "5px" }} />
                Eliminar cuenta
              </button>
            </>                     
          </div>
