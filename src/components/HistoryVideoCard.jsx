...
export function HistoryVideoCard({ video, onRemove, }) {
  ...
  return (
    <div className="group flex gap-3 p-2 rounded-lg hover:bg-gray-500 items-center">
      <Link to={`/video/${videoId}`}  className="relative w-28 h-20 sm:w-36 sm:h-24 md:w-40 md:h-24 shrink-0 rounded-lg overflow-hidden bg-gray-200 cursor-pointer">
      ...
      <Link to={`/video/${videoId}`}  className="flex-1 min-w-0 cursor-pointer">
        ...
        <p className="text-gray-400 text-xs line-clamp-1"> {views} views · {timestamp} </p>
        <p className="text-gray-200 text-xs">Watched {watchedAt}</p>
      </Link>
      <button onClick={(e) => { e.stopPropagation(); onRemove(); }}
        className="md:opacity-0 md:group-hover:opacity-100 md:transition-opacity opacity-100
        bg-red-600 text-white rounded-lg hover:text-gray-400 px-2 py-1"
      >
        <CloseIcon sx={{fontSize: "18px", color: "black", mr: "2px"}} />
        <span className="hidden md:inline-block">Remover</span>
      </button>
    </div>
  );
}
